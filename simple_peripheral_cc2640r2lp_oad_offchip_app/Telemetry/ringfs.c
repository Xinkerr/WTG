...
/** Retard a location to the end of the previous sector. */
static void _loc_retard_sector(struct ringfs *fs, struct ringfs_loc *loc)
{
    loc->slot = fs->slots_per_sector;
    loc->sector--;
    if (loc->sector < 0)
        loc->sector = fs->flash->sector_count;
}

/** Retard a location to the previous slot, retarding the sector too if needed. */
static void _loc_retard_slot(struct ringfs *fs, struct ringfs_loc *loc)
{
    loc->slot--;
    if (loc->slot < 0)
        _loc_retard_sector(fs, loc);
}
...
int ringfs_scan(struct ringfs *fs)
{
    ...
    for (int sector=0; sector<fs->flash->sector_count; sector++) {
        ...
        if (header.status == SECTOR_FORMATTING) {
            Log_info0("ringfs_scan: partially formatted partition\r\n");
            return -1;
        }
        ...
        if (header.status != SECTOR_FREE && header.status != SECTOR_IN_USE) {
            Log_info1("ringfs_scan: corrupted sector %d\r\n", sector);
            return -1;
        }
        ...
        if (header.version != fs->version) {
            Log_info1("ringfs_scan: incompatible version 0x%08"PRIx32"\r\n", header.version);
            return -1;
        }
    ...
    if (!free_seen) {
        Log_info0("ringfs_scan: invariant violated: no FREE sector found\r\n");
        return -1;
    }
...
int ringfs_append(struct ringfs *fs, const void *object)
{
    ...
    if (status == SECTOR_FREE) {
        ...
        Log_info0("ringfs_append: corrupted filesystem\r\n");
        return -1;
    }
...
int ringfs_fetch_LIFO(struct ringfs *fs, void *object)
{
    /* Retard backward in search of a valid slot. */
...
int ringfs_fastforward(struct ringfs *fs)
{
    fs->cursor = fs->write;
    return 0;
}
