...
struct ringfs_flash_partition
{
     ...
     */
    size_t (*program)(struct ringfs_flash_partition *flash, int address, const void *data, size_t size);
    /**
     * Read flash memory.
     * @param address Start address, in bytes.
     ...
     * @returns size on success, -1 on failure.
     */
    size_t (*read)(struct ringfs_flash_partition *flash, int address, void *data, size_t size);
};
...
/**
 * Fetch next object from the ring, newest-first. Retards read cursor.
 *
 * @param fs Initialized RingFS instance.
 * @param object Buffer to store retrieved object.
 * @returns Zero on success, -1 on failure.
...
/**
 * Fastforward the read cursor back to the newest  object.
 *
 * @param fs Initialized RingFS instance.
 * @returns Zero on success, -1 on failure.
 */
